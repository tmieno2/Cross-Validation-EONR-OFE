...
plot_gam_brf <- function(method2, data){
  gam<- data%>% filter(method %in% c("S-learner (GAM)"))%>%
    subset(., select=-c(id, repeats))
  
 brf<- data%>% filter(method %in% c(method2))%>%
    subset(., select=-c(id, repeats))
...
vis_gam_brf <- ggplot(gam_brf, aes(x=gam,y=brf, colour = "red")) + 
  geom_point()+geom_abline(slope=1, intercept = 0)+ xlim(if (x_3 < x_4) x_3 else x_4, if (x_1 > x_2) x_1 else x_2)+ 
...
plot_gam_rf <- function(method2, data){
  ...
  rf<- data%>% filter(method %in% c(method2))%>%
    subset(., select=-c(id, repeats))
...
plot_gam_linear <- function(method2, data){
  ...
  linear<- data%>% filter(method %in% c(method2))%>%
    subset(., select=-c(id, repeats))
