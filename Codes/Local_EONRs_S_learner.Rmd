---
title: "Modified_optimized_codes"
author: "Mona Mousavi"
date: "2023-01-11"
...
```{r}
EONR_all_models <- lapply(1:60, function(x) find_local_EONR_all(x, spatial_folds, data)) %>%
  rbindlist()
...
Find_MSE_Local_EONR <- function(method1, method2, data){
  mse <- data%>% filter(method %in% c(method1, method2))%>%
    group_by(repeats, id) %>%  summarise(mse=(diff(N_tgt))^2) %>% subset(., select=-c(id)) %>% 
    group_by(repeats) %>% summarise(mse_each_repeat=sum(mse))
...
mse_gam_brf <- Find_MSE_Local_EONR("S-learner (BRF)", "S-learner (GAM)", EONR_all_models) #10242.74
mse_gam_rf <- Find_MSE_Local_EONR("S-learner (RF)", "S-learner (GAM)", EONR_all_models)  #14438.68
mse_gam_linear <- Find_MSE_Local_EONR("S-learner (Linear)", "S-learner (GAM)", EONR_all_models)  #8003.313
