...
EONR_true <- function(n){ 
  ...
  true <- lapply(1:6, function(i){
    ...
    # true 
    True_EONR <- test_data%>% st_drop_geometry()%>%
...
true1<- true[1]%>%data.frame()%>% setnames(.,"repeat1")
true2 <- true[2]%>%data.frame()%>% setnames(.,"repeat2")
true3 <- true[3]%>%data.frame()%>% setnames(.,"repeat3")
true4 <- true[4]%>%data.frame()%>% setnames(.,"repeat4")
true5 <- true[5]%>%data.frame()%>% setnames(.,"repeat5")
true6 <- true[6]%>%data.frame()%>% setnames(.,"repeat6")
true7 <- true[7]%>%data.frame()%>% setnames(.,"repeat7")
true8 <- true[8]%>%data.frame()%>% setnames(.,"repeat8")
true9 <- true[9]%>%data.frame()%>% setnames(.,"repeat9")
true10 <-true[10]%>%data.frame()%>% setnames(.,"repeat10")

true_folds <- cbind(true1,true2,true3,true4,true5,
